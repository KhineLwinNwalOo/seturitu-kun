...
_LogDelete();
//_LogBackup();
_Log("[/user/company/pdf/create/tokishinseisho.php] start.");

_Log("[/user/company/pdf/create/tokishinseisho.php] POST = '".print_r($_POST,true)."'");
_Log("[/user/company/pdf/create/tokishinseisho.php] GET = '".print_r($_GET,true)."'");
_Log("[/user/company/pdf/create/tokishinseisho.php] SERVER = '".print_r($_SERVER,true)."'");
...
switch($loginInfo['usr_auth_id']){
	case AUTH_NON://権限無し
		...
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."' = '権限無し'");
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →自分のユーザー情報のみ表示する。");
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →ユーザーIDを設定する。");
		...
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →(ログイン)ユーザーID = '".$loginInfo['usr_user_id']."'");
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →(ログイン)会社ID = '".$relationCompanyId."'");
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →(パラメーター)ユーザーID = '".$userId."'");
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →(パラメーター)会社ID = '".$companyId."'");

		if ($userId != $loginInfo['usr_user_id']) $userId = $loginInfo['usr_user_id'];
		if ($companyId != $relationCompanyId) $companyId = $relationCompanyId;

		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →(処理対象)ユーザーID = '".$userId."'");
		_Log("[/user/company/pdf/create/tokishinseisho.php] {ログインユーザー権限処理} →(処理対象)会社ID = '".$companyId."'");
...
if (count($errorList) > 0) {
	//エラー有の場合
	_Log("[/user/company/pdf/create/tokishinseisho.php] end. ERR!");
...
$buf .= "金";
$buf .= " ";
$buf .= number_format(LICENSE_TAX_CMP);
$buf .= " ";
$buf .= "円";
$pdf->MultiCell(0,6,$buf,$border,"L");
$pdf->Ln();
...
//$buf .= "「株式の譲渡制限に関する規定」当会社の株式を譲渡により取得するには、代表取締役の承認を受けなければならない。";
//$buf .= "「株式の譲渡制限に関する規定」当会社の株式を譲渡するには、株主総会の承認を受けなければならない。";
$buf .= "「株式の譲渡制限に関する規定」当会社の株式を譲渡により取得するには、株主総会の承認を受けなければならない。";
$pdf->MultiCell(0,6,$buf,$border,"L");
...
_Log("[/user/company/pdf/create/tokishinseisho.php] end. OK!");

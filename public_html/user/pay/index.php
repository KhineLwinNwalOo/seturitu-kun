...
_LogDelete();
//_LogBackup();
_Log("[/user/pay/index.php] start.");


_Log("[/user/pay/index.php] \$_POST = '".print_r($_POST,true)."'");
_Log("[/user/pay/index.php] \$_GET = '".print_r($_GET,true)."'");
_Log("[/user/pay/index.php] \$_SERVER = '".print_r($_SERVER,true)."'");
_Log("[/user/pay/index.php] \$_SESSION = '".print_r($_SESSION,true)."'");
...
_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ start");
$tempFile = '../../common/temp_html/temp_base.txt';
_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (基本) HTMLテンプレートファイル = '".$tempFile."'");

$html = @file_get_contents($tempFile);
//"HTML"が存在する場合、表示する。
if ($html !== false && !_IsNull($html)) {
	_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (基本) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (基本) 【失敗】");
	$html .= "HTMLテンプレートファイルを取得できません。\n";
}


//$tempSidebarLoginFile = '../../common/temp_html/temp_sidebar_login.txt';
//_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) HTMLテンプレートファイル = '".$tempSidebarLoginFile."'");
//
//$htmlSidebarLogin = @file_get_contents($tempSidebarLoginFile);
////"HTML"が存在する場合、表示する。
//if ($htmlSidebarLogin !== false && !_IsNull($htmlSidebarLogin)) {
//	_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【成功】");
//} else {
//	//取得できなかった場合
//	_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【失敗】");
//}

$tempSidebarUserMenuFile = '../../common/temp_html/temp_sidebar_user_menu.txt';
_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) HTMLテンプレートファイル = '".$tempSidebarUserMenuFile."'");

$htmlSidebarUserMenu = @file_get_contents($tempSidebarUserMenuFile);
//"HTML"が存在する場合、表示する。
if ($htmlSidebarUserMenu !== false && !_IsNull($htmlSidebarUserMenu)) {
	_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【失敗】");
}

_Log("[/user/pay/index.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ end");
//HTMLテンプレートを読み込む。------------------------------------------------------- end
...
_Log("[/user/pay/index.php] \$_GET(詰め替え後) = '".print_r($_GET,true)."'");

//パラメーターを取得する。
$xmlName = XML_NAME_PAY;//XMLファイル名を設定する。
$id = null;
$step = null;
$stepId = null;
...
//		$step = (isset($_POST['condition']['_step_'])?$_POST['condition']['_step_']:null);
		...
		_Log("[/user/pay/index.php] {ログインユーザー権限処理} ユーザーID = '".$loginInfo['usr_user_id']."'");
		_Log("[/user/pay/index.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."'");
		...
		switch($loginInfo['usr_auth_id']){
			case AUTH_NON://権限無し
				...
				_Log("[/user/pay/index.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."' = '権限無し'");
				_Log("[/user/pay/index.php] {ログインユーザー権限処理} →自分のユーザー情報のみ表示する。");
				_Log("[/user/pay/index.php] {ログインユーザー権限処理} →ユーザーIDを設定する。");
				...
				_Log("[/user/pay/index.php] {ログインユーザー権限処理} →ユーザーID = '".$id."'");
				break;
		}
		...
		//入力値を取得する。
		$info = $_POST;
		_Log("[/user/pay/index.php] POST = '".print_r($info,true)."'");
		//バックスラッシュを取り除く。
		$info = _StripslashesForArray($info);
		_Log("[/user/pay/index.php] POST(バックスラッシュを取り除く。) = '".print_r($info,true)."'");
...
_Log("[/user/pay/index.php] \$_SERVER[\"REQUEST_METHOD\"] = '".$_SERVER["REQUEST_METHOD"]."'");
_Log("[/user/pay/index.php] XMLファイル名 = '".$xmlName."'");
_Log("[/user/pay/index.php] ターゲットID = '".$id."'");
...
//
//_Log("[/user/pay/index.php] ステップID = '".$step."'");
//_Log("[/user/pay/index.php] XMLファイル名(ステップID) = '".$xmlName."'");
//
...
if (!_IsNull($xmlName)) {
	...
	_Log("[/user/pay/index.php] XMLファイル = '".$xmlFile."'");
	$xmlList = _GetXml($xmlFile, $otherList);

	_Log("[/user/pay/index.php] XMLファイル配列 = '".print_r($xmlList,true)."'");
...
//			_Log("[/user/pay/index.php] XMLファイル = '".$bufXmlFile."'");
...
//
//
//			_Log("[/user/pay/index.php] XMLファイル配列(全XMLマージ後) = '".print_r($xmlList,true)."'");
//			_Log("[/user/pay/index.php] 法人印注文情報(全XMLマージ後) = '".print_r($info,true)."'");
//
//			$mode = 2;
//
//			break;
...
//				$body .= "\n";
				...
				_Log("[/user/pay/index.php] メール本文(_CreateMailAll) = '".$body."'");
				...
				$body .= "お支払い報告日時：".date("Y年n月j日 H時i分")."\n";
				...
				//$adminBody .= "\n";
				$adminBody .= "**************************************************************************************\n";
				$adminBody .= "『".$siteTitle."』にお支払い報告が入りました。\n";
				$adminBody .= "**************************************************************************************\n";
				$adminBody .= "\n";
				$adminBody .= $body;
				...
				$customerBody .= "\n";
				$customerBody .= "**************************************************************************************\n";
				$customerBody .= "この度は、『".$siteTitle."』にお支払い報告をしていただきありがとうございました。\n";
				...
				//管理者用タイトルを設定する。
				$adminTitle = "[".$siteTitle."] お支払い報告 (".$info['update']['tbl_user']['usr_family_name']." ".$info['update']['tbl_user']['usr_first_name']." 様)";
				//お客様用タイトルを設定する。
				$customerTitle = "[".$siteTitle."] お支払い報告ありがとうございました";

				mb_language("Japanese");

				//メール送信
				//お客様に送信する。
				$rcd = mb_send_mail($info['update']['tbl_pay']['pay_e_mail'], $customerTitle, $customerBody, "from:".$clientMail);

				//クライアントに送信する。
				$rcd = mb_send_mail($clientMail, $adminTitle, $adminBody, "from:".$info['update']['tbl_pay']['pay_e_mail']);

				//マスターに送信する。
				foreach($masterMailList as $masterMail){
					$rcd = mb_send_mail($masterMail, $adminTitle, $adminBody, "from:".$info['update']['tbl_pay']['pay_e_mail']);
				}
				...
				$message .= "\n";
				$message .= "\n";
				$message .= "この度は、『".$siteTitle."』にお支払い報告をしていただきありがとうございました。";
				$message .= "\n";
...
_Log("[/user/pay/index.php] POST(文字をHTMLエンティティに変換する。) = '".print_r($info,true)."'");

_Log("[/user/pay/index.php] mode = '".$mode."'");
...
$maincontent = null;
$maincontent .= "<h2>";
$maincontent .= "<img src=\"../../img/maincontent/pt_pay.jpg\" title=\"\" alt=\"お支払い報告\">";
$maincontent .= "</h2>";
$maincontent .= "\n";
...
_SetBreadcrumbs(PAGE_DIR_PAY, '', PAGE_TITLE_PAY, 3);
//パンくずリストを取得する。

...
_LogDelete();
//_LogBackup();
_Log("[/user/llc/pdf/index.php] start.");


_Log("[/user/llc/pdf/index.php] \$_POST = '".print_r($_POST,true)."'");
_Log("[/user/llc/pdf/index.php] \$_GET = '".print_r($_GET,true)."'");
_Log("[/user/llc/pdf/index.php] \$_SERVER = '".print_r($_SERVER,true)."'");
_Log("[/user/llc/pdf/index.php] \$_SESSION = '".print_r($_SESSION,true)."'");
...
_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ start");
$tempFile = '../../../common/temp_html/temp_base.txt';
_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (基本) HTMLテンプレートファイル = '".$tempFile."'");

$html = @file_get_contents($tempFile);
//"HTML"が存在する場合、表示する。
if ($html !== false && !_IsNull($html)) {
	_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (基本) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (基本) 【失敗】");
	$html .= "HTMLテンプレートファイルを取得できません。\n";
}


//$tempSidebarLoginFile = '../../../common/temp_html/temp_sidebar_login.txt';
//_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) HTMLテンプレートファイル = '".$tempSidebarLoginFile."'");
//
//$htmlSidebarLogin = @file_get_contents($tempSidebarLoginFile);
////"HTML"が存在する場合、表示する。
//if ($htmlSidebarLogin !== false && !_IsNull($htmlSidebarLogin)) {
//	_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【成功】");
//} else {
//	//取得できなかった場合
//	_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【失敗】");
//}

$tempSidebarUserMenuFile = '../../../common/temp_html/temp_sidebar_user_menu.txt';
_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) HTMLテンプレートファイル = '".$tempSidebarUserMenuFile."'");

$htmlSidebarUserMenu = @file_get_contents($tempSidebarUserMenuFile);
//"HTML"が存在する場合、表示する。
if ($htmlSidebarUserMenu !== false && !_IsNull($htmlSidebarUserMenu)) {
	_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【失敗】");
}

_Log("[/user/llc/pdf/index.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ end");
//HTMLテンプレートを読み込む。------------------------------------------------------- end
...
//		$step = (isset($_POST['condition']['_step_'])?$_POST['condition']['_step_']:null);
		...
		_Log("[/user/llc/pdf/index.php] {ログインユーザー権限処理} ユーザーID = '".$loginInfo['usr_user_id']."'");
		_Log("[/user/llc/pdf/index.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."'");
		...
		switch($loginInfo['usr_auth_id']){
			case AUTH_NON://権限無し
				...
				_Log("[/user/llc/pdf/index.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."' = '権限無し'");
				_Log("[/user/llc/pdf/index.php] {ログインユーザー権限処理} →自分のユーザー情報のみ表示する。");
				_Log("[/user/llc/pdf/index.php] {ログインユーザー権限処理} →ユーザーIDを設定する。");
				...
				_Log("[/user/llc/pdf/index.php] {ログインユーザー権限処理} →ユーザーID = '".$id."'");
				break;
		}
		...
		//入力値を取得する。
		$info = $_POST;
		_Log("[/user/llc/pdf/index.php] POST = '".print_r($info,true)."'");
		//バックスラッシュを取り除く。
		$info = _StripslashesForArray($info);
		_Log("[/user/llc/pdf/index.php] POST(バックスラッシュを取り除く。) = '".print_r($info,true)."'");

		//「半角カタカナ」を「全角カタカナ」に変換する。→メールで半角カナが文字化けするので。
		$info =_Mb_Convert_KanaForArray($info);
		_Log("[/user/llc/pdf/index.php] POST(「半角カタカナ」を「全角カタカナ」に変換する。) = '".print_r($info,true)."'");
...
_Log("[/user/llc/pdf/index.php] \$_SERVER[\"REQUEST_METHOD\"] = '".$_SERVER["REQUEST_METHOD"]."'");
_Log("[/user/llc/pdf/index.php] XMLファイル名 = '".$xmlName."'");
_Log("[/user/llc/pdf/index.php] ターゲットID = '".$id."'");
...
_Log("[/user/llc/pdf/index.php] POST(文字をHTMLエンティティに変換する。) = '".print_r($info,true)."'");

_Log("[/user/llc/pdf/index.php] mode = '".$mode."'");
...
$maincontent = null;
$maincontent .= "<h2>";
$maincontent .= "<img src=\"../../../img/maincontent/pt_user_llc_pdf.jpg\" title=\"\" alt=\"各種申請書類 印刷\">";
$maincontent .= "</h2>";
$maincontent .= "\n";
...
_SetBreadcrumbs(PAGE_DIR_LLC_PDF, '', PAGE_TITLE_LLC_PDF, 4);
//パンくずリストを取得する。
...
_Log("[/user/llc/pdf/index.php] end.");
echo $html;
...
//	$resBuf .= "※下の「4.」の「振込日」と同じ日付にしてください。";
	...
	$resBuf .= "<a href=\"../../../img/pdf/pdf_doc_01.pdf\" target=\"_blank\" title=\"合同会社設立登記申請書\">[印鑑説明]</a>";
	$resBuf .= "</div>";
...
//	$resBuf .= "</h5>";
	...
	$resBuf .= "<div class=\"exp\">";
	$resBuf .= "代表社員に就任することを承諾する書面です。自動で定款作成日と同じ日付で作成します。押印する<span class=\"attention\">印鑑は実印</span>を使用してください。(※代表社員の印鑑証明書のみを添付)";
	$resBuf .= "</div>";
...
//		$buf .= "登記書類の印刷は、<span class=\"attention\">必ず公証役場での電子定款「認証」後</span>に行ってください。<br />";
		$buf .= "登記書類の印刷は、<span class=\"attention\">行政書士から電子定款データをメールでお受け取り頂いた後</span>に行ってください。<br />";
		$buf .= "\n";
		$buf .= "<br />";
		$buf .= "\n";
...
//		$buf .= "登記の事由とはすべての準備が整った段階ということになりますので、<span class=\"attention\">資本金の振り込みが完了した時から2週間以内</span>となります。";
		...
		$buf .= "<li>定款・・・・・紙定款の場合は1通・電子定款の場合はCD-R又はFD(印刷した物ももって行くと良い)</li>";
//		$buf .= "<li>代表社員の印鑑証明・・・・・各1通(3ヶ月以内に発行されたもの)(法人の場合は法人の印鑑証明)</li>";
		$buf .= "<li>代表社員の印鑑証明・・・・・各1通(3ヶ月以内に発行されたものに限る)※法人の場合は法人の印鑑証明</li>";
//		$buf .= "<li>登録免許税・・・・・6万円(現金または収入印紙)(資本金が871万円以下の場合)</li>";
		$buf .= "<li>登録免許税・・・・・資本金が871万円以下の場合は6万円(現金または収入印紙)</li>";
		$buf .= "</ol>";
		$buf .= "</div>";
		$buf .= "\n";

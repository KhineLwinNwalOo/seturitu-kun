...
switch($loginInfo['usr_auth_id']){
	case AUTH_NON://権限無し
		...
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."' = '権限無し'");
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →自分のユーザー情報のみ表示する。");
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →ユーザーIDを設定する。");
		...
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →(ログイン)ユーザーID = '".$loginInfo['usr_user_id']."'");
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →(ログイン)会社ID = '".$relationCompanyId."'");
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →(パラメーター)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →(パラメーター)会社ID = '".$companyId."'");

		if ($userId != $loginInfo['usr_user_id']) $userId = $loginInfo['usr_user_id'];
		if ($companyId != $relationCompanyId) $companyId = $relationCompanyId;

		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →(処理対象)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/chosahokokusho.php] {ログインユーザー権限処理} →(処理対象)会社ID = '".$companyId."'");
...
$buf .= "の定款をもって設立時代表社員に選任されたので、会社法第46条の規定に基づいて調査した。その結果は次のとおりである。";
...
foreach ($companyInfo['tbl_company_promoter'] as $key => $promoterInfo) {
	...
	//発起人単位の小計
	$investmentCash = 0;				//現金の出資額
	$investmentInkind = 0;				//現物の出資額
...
$buf .= "であり、";
$buf .= "\n";
$buf .= "出資の目的である財産及びその価額は、下記のとおりである。";
$buf = mb_convert_kana($buf, 'N');
$pdf->MultiCell(0,6,$buf,$border,"L");

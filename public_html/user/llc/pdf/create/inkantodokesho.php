...
_LogDelete();
//_LogBackup();
_Log("[/user/llc/pdf/create/inkantodokesho.php] start.");

_Log("[/user/llc/pdf/create/inkantodokesho.php] POST = '".print_r($_POST,true)."'");
_Log("[/user/llc/pdf/create/inkantodokesho.php] GET = '".print_r($_GET,true)."'");
_Log("[/user/llc/pdf/create/inkantodokesho.php] SERVER = '".print_r($_SERVER,true)."'");
...
switch($loginInfo['usr_auth_id']){
	case AUTH_NON://権限無し
		...
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."' = '権限無し'");
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →自分のユーザー情報のみ表示する。");
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →ユーザーIDを設定する。");
		...
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →(ログイン)ユーザーID = '".$loginInfo['usr_user_id']."'");
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →(ログイン)会社ID = '".$relationCompanyId."'");
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →(パラメーター)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →(パラメーター)会社ID = '".$companyId."'");

		if ($userId != $loginInfo['usr_user_id']) $userId = $loginInfo['usr_user_id'];
		if ($companyId != $relationCompanyId) $companyId = $relationCompanyId;

		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →(処理対象)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/inkantodokesho.php] {ログインユーザー権限処理} →(処理対象)会社ID = '".$companyId."'");
...
if ($errFlag)  $errorList[] = "『代表社員の生年月日』を登録してください。";
//代理人
switch ($mode) {
	...
	if ($promoterInfo['cmp_prm_representative_partner_id'] == MST_REPRESENTATIVE_PARTNER_ID_YES) {
		...
		switch ($promoterInfo['cmp_prm_personal_type_id']) {
			case MST_PERSONAL_TYPE_ID_PERSONAL:
				...
				if (_IsNull($promoterInfo['cmp_prm_family_name_kana']) || _IsNull($promoterInfo['cmp_prm_first_name_kana'])) {
					$errFlag = true;
				}
...
if ($errFlag) $errorList[] = "『社員(出資者)』の『お名前』、『お名前(フリガナ)』、『住所』又は、『会社名(法人)』、『本店所在地』を登録してください。";
...
if (count($errorList) > 0) {
	//エラー有の場合
	_Log("[/user/llc/pdf/create/inkantodokesho.php] end. ERR!");


	$buf = "※PDFを作成するための情報が足りません。『合同会社設立LLC情報登録』画面で、情報を入力してください。又は、『定款認証』画面で、情報を入力してください。";
...
$pagecount = $pdf->setSourceFile("../../../../common/temp_pdf/inkantodokesho_llc.pdf");
...
//代表社員を取得する。
$repPromoterInfo = null;
foreach ($companyInfo['tbl_company_promoter'] as $key => $promoterInfo) {
	//代表社員ID="代表社員になる"が1人でもいるか？
	if ($promoterInfo['cmp_prm_representative_partner_id'] == MST_REPRESENTATIVE_PARTNER_ID_YES) {
		$repPromoterInfo = $promoterInfo;
		break;
	}
}
...
$nameKana = null;
$address = null;
switch ($repPromoterInfo['cmp_prm_personal_type_id']) {
	case MST_PERSONAL_TYPE_ID_PERSONAL:
		//個人
		$name .= $repPromoterInfo['cmp_prm_family_name'];
		$name .= " ";
		$name .= $repPromoterInfo['cmp_prm_first_name'];

		$nameKana .= $repPromoterInfo['cmp_prm_family_name_kana'];
		$nameKana .= " ";
		$nameKana .= $repPromoterInfo['cmp_prm_first_name_kana'];

		$address .= $mstPrefList[$repPromoterInfo['cmp_prm_pref_id']]['name'];
		$address .= $repPromoterInfo['cmp_prm_address1'];
		if (!_IsNull($repPromoterInfo['cmp_prm_address2'])) {
			if (!_IsNull($address)) $address .= " ";
			$address .= $repPromoterInfo['cmp_prm_address2'];
		}
		break;
	case MST_PERSONAL_TYPE_ID_CORPORATION:
		//法人(株式会社・有限会社のみ)
		$name .= $repPromoterInfo['cmp_prm_company_name'];

		$nameKana = null;

		$address .= $mstPrefList[$repPromoterInfo['cmp_prm_company_pref_id']]['name'];
		$address .= $repPromoterInfo['cmp_prm_company_address1'];
		if (!_IsNull($repPromoterInfo['cmp_prm_company_address2'])) {
			if (!_IsNull($address)) $address .= " ";
			$address .= $repPromoterInfo['cmp_prm_company_address2'];
		}
		break;
}
...
_Log("[/user/llc/pdf/create/inkantodokesho.php] end. OK!");

...
_LogDelete();
//_LogBackup();
_Log("[/user/llc/pdf/create/ininjo.php] start.");

_Log("[/user/llc/pdf/create/ininjo.php] POST = '".print_r($_POST,true)."'");
_Log("[/user/llc/pdf/create/ininjo.php] GET = '".print_r($_GET,true)."'");
_Log("[/user/llc/pdf/create/ininjo.php] SERVER = '".print_r($_SERVER,true)."'");
...
switch($loginInfo['usr_auth_id']){
	case AUTH_NON://権限無し
		...
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."' = '権限無し'");
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →自分のユーザー情報のみ表示する。");
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →ユーザーIDを設定する。");
		...
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →(ログイン)ユーザーID = '".$loginInfo['usr_user_id']."'");
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →(ログイン)会社ID = '".$relationCompanyId."'");
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →(パラメーター)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →(パラメーター)会社ID = '".$companyId."'");

		if ($userId != $loginInfo['usr_user_id']) $userId = $loginInfo['usr_user_id'];
		if ($companyId != $relationCompanyId) $companyId = $relationCompanyId;

		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →(処理対象)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/ininjo.php] {ログインユーザー権限処理} →(処理対象)会社ID = '".$companyId."'");
...
if (count($errorList) > 0) {
	//エラー有の場合
	_Log("[/user/llc/pdf/create/ininjo.php] end. ERR!");
...
switch ($mode) {
	case PDF_MODE_ININJO_PROMOTER:
		...
		$name = null;
		$address = null;
		switch ($promoterInfo4Agent['cmp_prm_personal_type_id']) {
			case MST_PERSONAL_TYPE_ID_PERSONAL:
				//個人
				$name .= $promoterInfo4Agent['cmp_prm_family_name'];
				$name .= " ";
				$name .= $promoterInfo4Agent['cmp_prm_first_name'];

				$address .= $mstPrefList[$promoterInfo4Agent['cmp_prm_pref_id']]['name'];
				$address .= $promoterInfo4Agent['cmp_prm_address1'];
				$address .= $promoterInfo4Agent['cmp_prm_address2'];
				break;
			case MST_PERSONAL_TYPE_ID_CORPORATION:
				//法人(株式会社・有限会社のみ)
				$name .= $promoterInfo4Agent['cmp_prm_company_name'];

				$address .= $mstPrefList[$promoterInfo4Agent['cmp_prm_company_pref_id']]['name'];
				$address .= $promoterInfo4Agent['cmp_prm_company_address1'];
				$address .= $promoterInfo4Agent['cmp_prm_company_address2'];
				break;
		}
...
_Log("[/user/llc/pdf/create/ininjo.php] end. OK!");

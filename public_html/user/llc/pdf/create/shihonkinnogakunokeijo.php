...
_LogDelete();
//_LogBackup();
_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] start.");

_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] POST = '".print_r($_POST,true)."'");
_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] GET = '".print_r($_GET,true)."'");
_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] SERVER = '".print_r($_SERVER,true)."'");
...
switch($loginInfo['usr_auth_id']){
	case AUTH_NON://権限無し
		...
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} 権限ID = '".$loginInfo['usr_auth_id']."' = '権限無し'");
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →自分のユーザー情報のみ表示する。");
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →ユーザーIDを設定する。");
		...
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →(ログイン)ユーザーID = '".$loginInfo['usr_user_id']."'");
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →(ログイン)会社ID = '".$relationCompanyId."'");
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →(パラメーター)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →(パラメーター)会社ID = '".$companyId."'");

		if ($userId != $loginInfo['usr_user_id']) $userId = $loginInfo['usr_user_id'];
		if ($companyId != $relationCompanyId) $companyId = $relationCompanyId;

		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →(処理対象)ユーザーID = '".$userId."'");
		_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] {ログインユーザー権限処理} →(処理対象)会社ID = '".$companyId."'");
...
if ($errFlag) $errorList[] = "『出資金』の『出資額』を登録してください。";



if (count($errorList) > 0) {
	//エラー有の場合
	_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] end. ERR!");
...
//	$totalInvestmentCash += $investmentCash * 10000;
//	$totalInvestmentInkind += $investmentInkind * 10000;
...
_Log("[/user/llc/pdf/create/shihonkinnogakunokeijo.php] end. OK!");

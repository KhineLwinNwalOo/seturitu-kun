...
//パラメーターを取得する。
$xmlName = XML_NAME_REGIST;//XMLファイル名を設定する。
$id = null;
	...
	if (_IsNull($message)) {
		...
		if ($res === false) {
			...
			if ($newFlag) {
				...
				//管理者用タイトルを設定する。
				$adminTitle = "[".$siteTitle."] ユーザー登録 (".$info['update']['tbl_user']['usr_family_name']." ".$info['update']['tbl_user']['usr_first_name']." 様)";
				//お客様用タイトルを設定する。
				$customerTitle = "[".$siteTitle."] ユーザー登録ありがとうございました";

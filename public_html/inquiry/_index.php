...
//			$message .= "保存しました。";
			...
			if ($_POST['go'] != "") {
				...
				$customerBody .= "\n";
				$customerBody .= "**************************************************************************************\n";
				$customerBody .= "この度は、『".$siteTitle."』にお問い合わせをしていただきありがとうございました。\n";
				...
				//管理者用タイトルを設定する。
				$adminTitle = "[".$siteTitle."] お問い合わせ (".$info['update']['tbl_user']['usr_family_name']." ".$info['update']['tbl_user']['usr_first_name']." 様)";
				//お客様用タイトルを設定する。
				$customerTitle = "[".$siteTitle."] お問い合わせありがとうございました";
				...
				$message .= "\n";
				$message .= "\n";
				$message .= "この度は、『".$siteTitle."』にお問い合わせをしていただきありがとうございました。";
				$message .= "\n";
...
_Log("[/inquiry/index.php] POST(文字をHTMLエンティティに変換する。) = '".print_r($info,true)."'");

_Log("[/inquiry/index.php] mode = '".$mode."'");
...
$maincontent = null;
$maincontent .= "<h2>";
$maincontent .= "<img src=\"../img/maincontent/pt_inquiry.jpg\" title=\"\" alt=\"お問い合わせ\">";
$maincontent .= "</h2>";
$maincontent .= "\n";
...
_SetBreadcrumbs(PAGE_DIR_INQUIRY, '', PAGE_TITLE_INQUIRY, 2);
//パンくずリストを取得する。

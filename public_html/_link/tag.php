...
_LogDelete();
//_LogBackup();
_Log("[/link/tag.php] start.");


_Log("[/link/tag.php] \$_POST = '".print_r($_POST,true)."'");
_Log("[/link/tag.php] \$_GET = '".print_r($_GET,true)."'");
_Log("[/link/tag.php] \$_SERVER = '".print_r($_SERVER,true)."'");
_Log("[/link/tag.php] \$_SESSION = '".print_r($_SESSION,true)."'");
...
//	_Log("[/link/tag.php] ログインしていないなのでログイン画面を表示する。");
//	_Log("[/link/tag.php] end.");
//	//ログイン画面を表示する。
...
_Log("[/link/tag.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ start");
$tempFile = '../common/temp_html/temp_base.txt';
_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (基本) HTMLテンプレートファイル = '".$tempFile."'");

$html = @file_get_contents($tempFile);
//"HTML"が存在する場合、表示する。
if ($html !== false && !_IsNull($html)) {
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (基本) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (基本) 【失敗】");
	$html .= "HTMLテンプレートファイルを取得できません。\n";
}


$tempSidebarLoginFile = '../common/temp_html/temp_sidebar_login.txt';
_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) HTMLテンプレートファイル = '".$tempSidebarLoginFile."'");

$htmlSidebarLogin = @file_get_contents($tempSidebarLoginFile);
//"HTML"が存在する場合、表示する。
if ($htmlSidebarLogin !== false && !_IsNull($htmlSidebarLogin)) {
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【失敗】");
}

$tempSidebarUserMenuFile = '../common/temp_html/temp_sidebar_user_menu.txt';
_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) HTMLテンプレートファイル = '".$tempSidebarUserMenuFile."'");

$htmlSidebarUserMenu = @file_get_contents($tempSidebarUserMenuFile);
//"HTML"が存在する場合、表示する。
if ($htmlSidebarUserMenu !== false && !_IsNull($htmlSidebarUserMenu)) {
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【失敗】");
}


$tempMaincontentLinkFile = '../common/temp_html/temp_maincontent_link_tag.txt';
_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (メインコンテンツリンク) HTMLテンプレートファイル = '".$tempMaincontentLinkFile."'");

$htmlMaincontentLink = @file_get_contents($tempMaincontentLinkFile);
//"HTML"が存在する場合、表示する。
if ($htmlMaincontentLink !== false && !_IsNull($htmlMaincontentLink)) {
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (メインコンテンツリンク) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/tag.php] {HTMLテンプレートを読み込み} (メインコンテンツリンク) 【失敗】");
}




_Log("[/link/tag.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ end");
//HTMLテンプレートを読み込む。------------------------------------------------------- end
...
//ページタイトル
$pageTitle = PAGE_TITLE_LINK_BANNER;
...
_Log("[/link/tag.php] POST(文字をHTMLエンティティに変換する。) = '".print_r($info,true)."'");

_Log("[/link/tag.php] mode = '".$mode."'");
...
_SetBreadcrumbs(PAGE_DIR_LINK_BANNER, '', PAGE_TITLE_LINK_BANNER, 3);
//パンくずリストを取得する。

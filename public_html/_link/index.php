...
_LogDelete();
//_LogBackup();
_Log("[/link/index.php] start.");


_Log("[/link/index.php] \$_POST = '".print_r($_POST,true)."'");
_Log("[/link/index.php] \$_GET = '".print_r($_GET,true)."'");
_Log("[/link/index.php] \$_SERVER = '".print_r($_SERVER,true)."'");
_Log("[/link/index.php] \$_SESSION = '".print_r($_SESSION,true)."'");
...
//	_Log("[/link/index.php] ログインしていないなのでログイン画面を表示する。");
//	_Log("[/link/index.php] end.");
//	//ログイン画面を表示する。
...
_Log("[/link/index.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ start");
$tempFile = '../common/temp_html/temp_base.txt';
_Log("[/link/index.php] {HTMLテンプレートを読み込み} (基本) HTMLテンプレートファイル = '".$tempFile."'");

$html = @file_get_contents($tempFile);
//"HTML"が存在する場合、表示する。
if ($html !== false && !_IsNull($html)) {
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (基本) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (基本) 【失敗】");
	$html .= "HTMLテンプレートファイルを取得できません。\n";
}


$tempSidebarLoginFile = '../common/temp_html/temp_sidebar_login.txt';
_Log("[/link/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) HTMLテンプレートファイル = '".$tempSidebarLoginFile."'");

$htmlSidebarLogin = @file_get_contents($tempSidebarLoginFile);
//"HTML"が存在する場合、表示する。
if ($htmlSidebarLogin !== false && !_IsNull($htmlSidebarLogin)) {
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (サイドメニューログイン) 【失敗】");
}

$tempSidebarUserMenuFile = '../common/temp_html/temp_sidebar_user_menu.txt';
_Log("[/link/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) HTMLテンプレートファイル = '".$tempSidebarUserMenuFile."'");

$htmlSidebarUserMenu = @file_get_contents($tempSidebarUserMenuFile);
//"HTML"が存在する場合、表示する。
if ($htmlSidebarUserMenu !== false && !_IsNull($htmlSidebarUserMenu)) {
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (サイドメニュー会員メニュー) 【失敗】");
}


$tempMaincontentLinkFile = '../common/temp_html/temp_maincontent_link.txt';
_Log("[/link/index.php] {HTMLテンプレートを読み込み} (メインコンテンツリンク) HTMLテンプレートファイル = '".$tempMaincontentLinkFile."'");

$htmlMaincontentLink = @file_get_contents($tempMaincontentLinkFile);
//"HTML"が存在する場合、表示する。
if ($htmlMaincontentLink !== false && !_IsNull($htmlMaincontentLink)) {
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (メインコンテンツリンク) 【成功】");
} else {
	//取得できなかった場合
	_Log("[/link/index.php] {HTMLテンプレートを読み込み} (メインコンテンツリンク) 【失敗】");
}




_Log("[/link/index.php] {HTMLテンプレートを読み込み} ━━━━━━━━━━━━━━━ end");
//HTMLテンプレートを読み込む。------------------------------------------------------- end
...
_Log("[/link/index.php] POST(文字をHTMLエンティティに変換する。) = '".print_r($info,true)."'");

_Log("[/link/index.php] mode = '".$mode."'");
...
//メタ情報
$html = str_replace ('{keywords}', PAGE_KEYWORDS_LINK, $html);
$html = str_replace ('{description}', PAGE_DESCRIPTION_LINK, $html);
//コンテンツ
